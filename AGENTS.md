...
- Anonyme Nutzungsstatistik über Vercel Web Analytics: `src/analytics.js` (Seitenaufrufe, Events «Karte gedreht» und «Unterricht geöffnet» mit Karte, Sprache, Ort; Query-Parameter entfernt; Standalone-Starts unter `/app/standalone/`), eingebunden in den drei Einstiegen. Events nur im Pro-Plan sichtbar. Die Datenschutzerklärung nennt Vercel als Host und beschreibt die Statistik — bei Änderungen in allen sechs Sprachen nachziehen.
