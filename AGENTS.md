...
- Vite/React-PWA mit Landing unter `/` und installierbarer App unter `/app/`; kein Backend, Persistenz nur in `localStorage`.
...
- SEO/GEO liegt unter `src/seo/`; der Build erzeugt `robots.txt`, `sitemap.xml`, `llms.txt`, OG-Bild sowie Seiten unter `/dimensionen/{id}/` und `/kompetenzen/{id}/`. Indexiert: Landing, Projekt, Kontakt, Dimensionen, Kompetenzen; `/app/`, Datenschutz und Nutzungsbedingungen bleiben noindex.
